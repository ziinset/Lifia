...
@section('scripts')
    @auth
        <script>window.LIFIA_AUTH = { loggedIn: true };</script>
    @else
        <script>window.LIFIA_AUTH = { loggedIn: false };</script>
    @endauth
    <script>
        (function() {
            ...
            document.getElementById('calcBtn').addEventListener('click', function() {
                ...
                try {
                    const payload = {
                        height_cm: hCm,
                        weight_kg: w,
                        bmi: parseFloat((Math.round(bmi * 10) / 10).toFixed(1)),
                        measured_at: (function(){
                            ...
                            const local = new Date(now.getTime() - tzOffsetMs)
                                ...
                                .slice(0,19)
                                .replace('T',' ');
                            return local; // 'YYYY-MM-DD HH:mm:ss' in local time
                        })()
                    };

                    // Simpan localStorage sebagai fallback
                    try {
                        localStorage.setItem('lifia_bmi_result', JSON.stringify({
                    ...
                    // Kirim ke server jika login (CSRF token tersedia)
                    ...
                    if (csrf) {
                        fetch('{{ url('/bmi') }}', {
                            ...
                            if (res.ok) {
                                // tersimpan
                                console.log('BMI tersimpan ke server');
                            } else if (res.status === 401) {
                                alert('Untuk menyimpan hasil BMI ke akun Anda, silakan login terlebih dahulu.');
                            } else if (res.status === 419) {
                                alert('Sesi kadaluarsa. Refresh halaman lalu coba lagi.');
                            } else {
                                const msg = await res.text();
                                console.error('Gagal menyimpan BMI:', res.status, msg);
                                alert('Gagal menyimpan BMI ke server.');
                            }
                        }).catch((err) => {
                            console.error('Koneksi gagal saat menyimpan BMI', err);
                        });
                    }
                } catch (e) {
                    // noop
