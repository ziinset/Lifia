    ...
    <nav class="navbar">
        <div class="navbar-container">
            ...
            <div class="navbar-links" id="navbarLinks">
                            ...
                            @if(isset($globalCategories) && $globalCategories->count())
                                @foreach($globalCategories as $cat)
                                    @php
                                        ...
                                        $active = request()->is('kategori/' . $cat->slug . '*');
                                        $icon = $cat->icon ?: 'fas fa-folder';
                                    @endphp
                                    <a href="{{ $href }}" class="dropdown-item{{ $active ? ' navbar-active' : '' }}" data-nav="{{ $cat->slug }}">
                                        <i class="{{ $icon }}"></i>
                ...
                @endphp
                <a href="{{ Auth::check() && Auth::user()->role === 'admin' ? route('admin.dashboard') : route('profil') }}" class="navbar-account" style="display:flex; align-items:center; gap:10px; text-decoration:none; color:#fff; border:1.4px solid #fff; border-radius:30px; padding:7px 14px; background:transparent;">
                ...
                @else
                <a href="{{ route('login', ['redirect_to' => request()->getRequestUri()]) }}" class="navbar-login" data-nav="login">
                    Login
                </a>
                @endauth
            </div>
